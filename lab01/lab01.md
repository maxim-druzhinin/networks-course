...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Английский (en-US,en;q=0.9)
   - Инфа в User-Agent (версия браузера, операционная система и платформа); ещё есть Accept-Encoding
...
6. Сколько байтов контента возвращается вашему браузеру?
   - 128
  <img width="1358" height="986" alt="Screenshot 2026-02-24 at 13 55 14" src="https://github.com/user-attachments/assets/80f594c8-ac83-4ed7-918e-bcf3952f6c3f" />
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да вернул
   - File Data: 371 bytes и дальше там прям текст (Lined-base-text-data)
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Есть такое; значение If-Modified-Since: Tue, 28 Oct 2025 05:59:01 GMT
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - 304 Not Modified
   - Нет не вернул, нет Lined-base-text-data и ответ стал меньше
  
<img width="1406" height="671" alt="Screenshot 2026-02-24 at 15 08 53" src="https://github.com/user-attachments/assets/21df01d2-a46c-4cde-933b-f2792373c455" />
...
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   ...
   - Нет
     ...
     <img width="1381" height="953" alt="Screenshot 2026-02-24 at 15 17 28" src="https://github.com/user-attachments/assets/021bbec8-05cd-43ea-9c31-9373f9b87df4" />
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3 запроса get
   - Один на `kurose.cslash.net` и два на `gaia.cs.umass.edu`; 
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Кажется что последовательно, т.к get и получения идут друг за другом и не пересекаются. Нет такого, что было отправлен get раньше чем получен ответ. Но есть гипотеза, что не факт что так всегда будет.
  
     <img width="1378" height="965" alt="Screenshot 2026-02-24 at 15 24 06" src="https://github.com/user-attachments/assets/5e7c6ea1-1845-4155-aa50-74e9b6dd1e7f" />
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - Authorization: Basic d2lyZXNoYXJrLXN0dWRlbnRzOm5ldHdvcms c Credentials: wireshark-students:network
  <img width="1382" height="918" alt="Screenshot 2026-02-24 at 15 33 56" src="https://github.com/user-attachments/assets/39f36ff6-cdd8-42ee-a37a-5ca245059e99" />
