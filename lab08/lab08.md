...
#### Демонстрация работы
<img width="1680" height="1064" alt="Screenshot 2026-04-25 at 15 15 05" src="https://github.com/user-attachments/assets/c50a8e6f-f295-4191-93db-9f73a5d37b44" />
...
<img width="1653" height="594" alt="Screenshot 2026-04-25 at 15 47 16" src="https://github.com/user-attachments/assets/2d5950e0-3f38-447c-8031-eb714584cf42" />
...
<img width="749" height="979" alt="Screenshot 2026-04-25 at 15 30 34" src="https://github.com/user-attachments/assets/63c517c5-1230-495c-92bf-a06448ad76ec" />
...
#### Решение
За RTT окно растёт на 1 MSS, т.е. $T = \frac{W}{2}$

Среднее окно в свою очередь: $\frac{3W}{4}$, а значит и средняя пропускная способность $R=\frac{3W}{4}$

Тогда: $W = \frac{4R}{3} \;\Rightarrow\; T = \frac{W}{2} = \frac{2R}{3}$
...
#### Решение
Обозначим $a = \frac{S}{R}$. Окна отправляются так $1+2+4+8=15$

Базовое время (как бы без простоев): $T_0 = 2RTT + 15a$ (установление TCP-соединения + запрос клиента и приход первого сегмента ответа -- $2RTT$, ну и $15a$ -- все данные)


1) после 1 и 2 запаздывает

$T = T_0 +RTT + (RTT-a) = 4RTT +14a$

3) после 1, 2 и 4

$T = T_0+ RTT + (RTT-a) +(RTT-3a) = 5RTT + 11a$
...
#### Решение
$W_k = \frac{W}{2}(1+a)^k$. До достижения $W$: $\frac{W}{2}(1+a)^T = W \Rightarrow (1+a)^T = 2$ причем $T = \log_{1+a} 2$, что константа

Количество переданных сегментов за цикл: $\frac{W}{2}\left(1 + (1+a) + \dots + (1+a)^T \right)$. Это геом. прогр. $\frac{W}{2} \cdot \frac{(1+a)^{T+1}-1}{a}$

Так как $(1+a)^T = 2$, то $(1+a)^{T+1} = 2(1+a)$

Тогда $Cnt = \frac{W}{2} \cdot \frac{2(1+a)-1}{a}= \frac{W(2a+1)}{2a}$

А частота потерь $L = \frac{1}{Cnt} = \frac{2a}{W(2a+1)}$, где $T = const$
...
#### Решение
Сначала клиент устанавливает TCP-соединение с внешним сервером ($RTT_{FE}$). Далее внешний сервер запрашивает данные у дата-центра по уже установленному соединению с большим окном, поэтому медленного старта нет ($RTT_{BE}$). Ответ клиенту передаётся за три окна ($3RTT_{FE}$). Также  время обработки ($t$)

Проссумируем и получим $T = 4RTT_{FE} + RTT_{BE} + t$
